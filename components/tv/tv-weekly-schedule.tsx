...
export function TvWeeklySchedule({ week }: { week: TvDaySummary[] }) {
  ...
  return (
    <footer className="grid shrink-0 grid-cols-[2fr_3fr] gap-4 border-t-2 border-border bg-card px-6 py-4">
      <div className="grid grid-cols-7 gap-2">
        {week.map((day) => {
          ...
          return (
            ...
            >
              <span className={cn("text-xs font-bold uppercase tracking-wide", isToday ? "text-secondary" : "text-muted-foreground")}>
                {day.label.slice(0, 3)}
              </span>
              <span className="font-mono text-2xl font-extrabold tabular-nums">{day.totalOrders}</span>
      ...
      <div className="min-w-0 rounded-xl border border-border p-3">
        <div className="mb-1.5 text-sm font-extrabold uppercase tracking-wide text-muted-foreground">
        ...
        {selectedDay.orders.length === 0 ? (
          <div className="text-sm font-medium text-muted-foreground">No orders scheduled.</div>
        ) : (
