...
interface ItemReadinessDialogProps {
  ...
  /** Outsourced workers' "done" action is ready_internal_pickup, never ready_delivery — so they never see the delivery-provider prompt below. */
  isOutsourced: boolean;
}

export function ItemReadinessDialog({ open, onOpenChange, job, isOutsourced }: ItemReadinessDialogProps) {
  const queryClient = useQueryClient();
  const [pendingId, setPendingId] = useState<string | null>(null);
  const [pendingLastItem, setPendingLastItem] = useState<string | null>(null);
  ...
  const commitToggle = (itemId: string, next: boolean, deliveryProvider?: OrderDeliveryProvider) => {
    const willCompleteAll = next && remainingCount === 1 && !rows.find((r) => r.id === itemId)?.isReady;
    setPendingId(itemId);
    toggleJobItemReady(job.id, itemId, next, deliveryProvider)
      .then(() => {
  ...
  const handleToggle = (itemId: string, next: boolean) => {
    const willCompleteAll = next && remainingCount === 1 && !rows.find((r) => r.id === itemId)?.isReady;
    // Checking the very last item is what auto-advances a delivery order
    // past ready_delivery with no separate button click to hang the
    // "Who's delivering this?" prompt off of (see status-actions.tsx for
    // the single-item equivalent) — so ask here instead, before the toggle
    // actually commits.
    if (willCompleteAll && job.fulfillmentType === "delivery" && !isOutsourced) {
      setPendingLastItem(itemId);
      return;
    }
    commitToggle(itemId, next);
  };
  ...
  return (
    <>
      ...
      <DeliveryProviderDialog
        open={!!pendingLastItem}
        onOpenChange={(nextOpen) => !nextOpen && setPendingLastItem(null)}
        onChoose={(provider) => {
          const itemId = pendingLastItem;
          setPendingLastItem(null);
          if (itemId) commitToggle(itemId, true, provider);
        }}
      />
    </>
  );
