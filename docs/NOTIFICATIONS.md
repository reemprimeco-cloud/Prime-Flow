...
Meta rejects templates with too many variables relative to the amount of fixed text around them ("too many variables for its length") — `admin_order_status_changed`'s original terse 4-variable one-liner hit this. The lesson is to pad short, high-variable-count messages with more fixed context rather than dropping information: the approved version carries four variables (employeeName, orderNumber, customerName, statusLabel) inside a much wordier sentence, plus the dashboard link (`https://primeflowboard.netlify.app/dashboard`) as plain static text since it never varies. `productName` is the one deliberately left out — the customer's name identifies the job at a glance far better than the product does.

**Changing a template's variable count means re-submitting it to Meta and swapping the Content SID *before* deploying the code.** Positions are matched by number, not name, so shipping 4 variables at a SID approved for 3 doesn't error — it silently renders the wrong value in each slot. This happened in production: adding `customerName` as the new `{{3}}` pushed `statusLabel` into a `{{4}}` the approved template didn't have, and a real alert read `moved order #1007 to status "Devon"`. The mapping was reverted to three variables until the wider template is approved.

The customer's name is already in the **freeform** body of both admin templates (`lib/notifications/templates.ts`), so `admin_order_note_added` — which has no Content SID and always sends freeform — carries it today. `admin_order_status_changed` picks it up only once its 4-variable template is approved and its SID replaced.
