...
**Changing a template's variable count means the new Content SID and the `buildVariables` mapping must ship in the same deploy.** Positions are matched by number, not name, so a mismatch doesn't error — it silently renders the wrong value in each slot. This bit us in production: `customerName` was added as the new `{{3}}` while the approved template still had three slots, which pushed `statusLabel` into a `{{4}}` that didn't exist, and a real alert read `moved order #1007 to status "Devon"`. Both admin alerts now name the customer, via `prime_admin_tap_notify3` (4 slots) for the status change and freeform text for `admin_order_note_added`, which has no Content SID.
