...
**Changing a template's variable count means re-submitting it to Meta and swapping the Content SID before deploying the code.** Positions are matched by number, not name, so shipping 4 variables at a SID approved for 3 doesn't error — it silently renders the wrong value in each slot.
