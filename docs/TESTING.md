...
| Notifications | `lib/notifications/templates.test.ts`, `lib/notifications/constants.test.ts` | Every template (all 12: 7 customer + 5 employee) rendered in both languages; variable interpolation and company-name/pickup-location defaults verified; `normalizeNotificationPreferences` tested for partial/legacy/null input handling. |
