...
- **No persisted fulfillment method.** The schema has no "pickup vs. delivery" field on an order — it was never needed before Phase 7. Today's Deliveries/Pickups (Operations Dashboard) and the Calendar's pickup/delivery framing both infer it from current status: `ready_pickup`/`collected` = pickup-track, `ready_delivery`/`delivered` = delivery-track. Good enough for the current workflow, but if an order's fulfillment method needs to be known before either of those statuses is reached, this would need a real column.
