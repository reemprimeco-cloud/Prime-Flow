...
- Multi-item orders (`order_items`) don't yet have independent per-item status — the whole order still moves through the Status Engine/Employee Dashboard/TV Dashboard as one unit, with `order_items` only tracking each item's spec and assigned employee. Splitting status per item is a larger production-floor rework (Status Engine transition graph, Employee Dashboard job cards, TV Dashboard columns, and per-item notifications) intentionally deferred to a follow-up pass.
