...
The Specifications section collects **Product, Quantity, and a single free-text "Order details" box** — the separate Paper / Paper Size / Finishing inputs are gone from both the admin order form and the public `/order-request` form, and from the per-item blocks in each. Real jobs rarely split cleanly into those three (a UV DTF sticker has a roll width and sheet length; a business card has paper weight), and typing everything into one box is faster on a phone than tabbing through three that are usually half-empty.

That box writes to the existing `orders.finishing` / `order_items.finishing` column (max raised 300 → 1000 chars) rather than adding a new one — the column name is now narrower than what it holds, but renaming it across the schema, generated types, notification context, and every display component isn't worth the churn. `paper` / `paper_size` are still read and displayed wherever they're non-empty, since orders created before this change have real values in them; they're simply no longer collected. Display components (`job-card`, `order-card`, `order-detail-drawer`, `item-readiness-dialog`) render each of the three conditionally instead of falling back to an em dash, so a newer order shows Qty and its details rather than two empty columns.
